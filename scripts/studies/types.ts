 ...
 *  When a table spans two calendar years (e.g. SEP 23 .. OCT 24) the same month is summed and spansTwoYears is true. */
export interface MonthlyCosts {
  basis: string;                    // 'per acre' in nearly every study; 'per 1,000 lf' for hedgerow studies
  cashCostsPerAcre: number[] | null; // 12 entries, TOTAL CASH COSTS row (or operating + cash overhead rows); null when the table only spreads operating costs
  operatingPerAcre: number[] | null; // 12 entries, TOTAL OPERATING COSTS row, when present
  harvestMonths: boolean[];         // 12 entries: true where the harvest total row has a nonzero cost
  totalCheck: number;               // the TOTAL column printed by the study for the row named in checkedRow
  checkedRow: string;               // which row(s) totalCheck and the reconciliation refer to
  columns: string[];                // the month header tokens as printed, in table order
